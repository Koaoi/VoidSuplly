...
<div class="min-h-screen flex items-center justify-center px-4 py-24">
    <div class="w-full max-w-md animate-slide-up">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-void-white">Masuk ke akunmu</h1>
            <p class="text-sm text-void-gray mt-2">Belum punya akun?
                <a href="{{ route('register') }}" class="text-void-accent hover:underline font-medium">Daftar sekarang</a>
            </p>
        </div>
        ...
        <div class="bg-void-card border border-void-border rounded-2xl p-8">
            ...
            <a href="{{ route('auth.google') }}"
               class="flex items-center justify-center gap-3 w-full px-4 py-3 bg-void-dark border border-void-border rounded-xl text-sm font-medium text-void-light hover:border-void-muted hover:bg-void-muted/30 transition-all duration-200 group">
            ...
            <div class="relative my-6">
                ...
                <div class="relative flex justify-center">
                    <span class="bg-void-card px-4 text-xs text-void-gray">atau login dengan email</span>
                </div>
            </div>

            {{-- Error Global --}}
            @if($errors->has('email') && !$errors->has('password'))
                <div class="mb-5 flex items-center gap-2 bg-red-500/10 border border-red-500/30 text-red-400 text-sm px-4 py-3 rounded-xl">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <label for="password" class="block text-xs font-semibold text-void-light tracking-wider uppercase">
                            Password
                        </label>
                        <a href="#" class="text-xs text-void-gray hover:text-void-accent transition-colors">Lupa password?</a>
                    </div>
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" id="remember" name="remember"
                           class="w-4 h-4 rounded border-void-border bg-void-dark text-void-accent
                                  focus:ring-0 focus:ring-offset-0 cursor-pointer">
                    <label for="remember" class="text-sm text-void-gray cursor-pointer select-none">
                        Ingat saya
                    </label>
                </div>
